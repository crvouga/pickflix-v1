...
describe('/lists/{list-id}/items/{list-item-id}', () => {
  describe('DELETE', () => {
    it('deletes list item', async done => {
      ...
      const listItem = await ListLogic.addListItem({
        listId: list.id,
        tmdbId: '42',
        tmdbMediaType: 'movie',
      });
...
describe('/lists/{list-id}', () => {
  ...
  describe('DELETE', () => {
    it('deletes list', async done => {
      ...
      await supertest(app).delete(`/lists/${list.id}`).expect(204);
      done();
    });
  });
});
